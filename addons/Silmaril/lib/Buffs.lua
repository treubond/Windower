do
    ...
    function run_buffs(data)
        ...
        -- Get the protection table
        local cache = get_name_cache()
        local protect = get_protection()
        ...
        for k = 0, 5 do
            ...
            if Uid then
                Character = windower.ffxi.get_mob_by_id(Uid)
            end

            if Character then
                -- Limit packets being sent and dont send self (grabs it from status update)
                if Character.name ~= get_player_name() then
                    formattedString = "partybuffs_"..Character.name.."_"
                    for i = 1, 32 do
    ...
    function get_all_buffs()
        local formattedString = get_player_name()..";buffdata_"
        ...
        for id, buff in pairs(all_buffs) do
            ...
            if buff.id and tonumber(buff.id) > tonumber(all_buff_count) then
