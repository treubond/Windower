...
elements =  M('Fire', 'Ice', 'Air', 'Dark', 'Light', 'Earth', 'Lightning', 'Water')
...
function update_active_ja(name, gain)
    Buff['Entrust'] = buffactive['Entrust'] or false
	Buff['Collimated Fervor'] = buffactive['Collimated Fervor'] or false
end
if use_UI == true then
    setupTextWindow()
else
...
function precast(spell)
	...
	-- Change target if Entrust is up
	if spell.name:match('Indi') and Buff['Entrust'] and spell.target.type == "SELF" then
		cancel_spell()
		send_command('input /ma '..spell.name..' <stpt>')
	end
...
function midcast(spell)
    ...
    elseif sets.me[spell.name] then
		...
		--TODO: Reintroduce Obi rules for magic WS
    end
